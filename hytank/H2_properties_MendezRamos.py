"""
@File    :   H2_properties_MendezRamos.py
@Date    :   2023/07/26
@Author  :   Eytan Adler
@Description : Hydrogen property surrogate models from Eugina Mendez Ramos's thesis.
               While this is no longer used directly by the hydrogen boil off model,
               it is kept because it can handle complex numbers, which enables
               the use of complex step derivatives to check the accuracy of
               the boil off model partial derivatives.
"""
...
def gh2_P(rho, T, deriv=False):
    """
    Compute pressure of hydrogen gas using the ideal gas law.
    ...
    if deriv:
        return (
            T * UNIVERSAL_GAS_CONST / MOLEC_WEIGHT_H2 + rho * 0,  # rho * 0 is to make sure shape is correct
            rho * UNIVERSAL_GAS_CONST / MOLEC_WEIGHT_H2 + T * 0,
        )
    return rho * T * UNIVERSAL_GAS_CONST / MOLEC_WEIGHT_H2


def gh2_rho(P, T, deriv=False):
    """
    Compute density of hydrogen gas using the ideal gas law.
    ...
    if deriv:
        return (
            1 / T / UNIVERSAL_GAS_CONST * MOLEC_WEIGHT_H2 + P * 0,
            -P / T**2 / UNIVERSAL_GAS_CONST * MOLEC_WEIGHT_H2,
        )
    return P / T / UNIVERSAL_GAS_CONST * MOLEC_WEIGHT_H2
...
def gh2_cv(P, T, deriv=False):
    ...
    if deriv:
        ...
        H1_PMPa = 0.275976950953691 * (
            1 - np.tanh((0.3990654435825 + 0.275976950953691 * P_MPa + -0.0364541001467881 * T)) ** 2
        )
        H1_T = -0.0364541001467881 * (
            1 - np.tanh((0.3990654435825 + 0.275976950953691 * P_MPa + -0.0364541001467881 * T)) ** 2
        )
        H2_PMPa = -2.62940079620686 * (
            1 - np.tanh((-4.86826687998567 + -2.62940079620686 * P_MPa + 0.216404289884235 * T)) ** 2
        )
        H2_T = 0.216404289884235 * (
            1 - np.tanh((-4.86826687998567 + -2.62940079620686 * P_MPa + 0.216404289884235 * T)) ** 2
        )
        H3_PMPa = 0.269571399186571 * (
            1 - np.tanh((1.12548975067467 + 0.269571399186571 * P_MPa + -0.0349250996335972 * T)) ** 2
        )
        H3_T = -0.0349250996335972 * (
            1 - np.tanh((1.12548975067467 + 0.269571399186571 * P_MPa + -0.0349250996335972 * T)) ** 2
        )
        H4_PMPa = -3.1347851189393 * (
            1 - np.tanh((-5.57528770123298 + -3.1347851189393 * P_MPa + 0.276378205247149 * T)) ** 2
        )
        H4_T = 0.276378205247149 * (
            1 - np.tanh((-5.57528770123298 + -3.1347851189393 * P_MPa + 0.276378205247149 * T)) ** 2
        )
        H5_PMPa = -0.415187001771689 * (
            1 - np.tanh((-0.768555052808218 + -0.415187001771689 * P_MPa + 0.0303660811195935 * T)) ** 2
        )
        H5_T = 0.0303660811195935 * (
            1 - np.tanh((-0.768555052808218 + -0.415187001771689 * P_MPa + 0.0303660811195935 * T)) ** 2
        )
        H6_PMPa = -1.67288064390926 * (
            1 - np.tanh((0.271020942398391 + -1.67288064390926 * P_MPa + 0.0673489649704582 * T)) ** 2
        )
        H6_T = 0.0673489649704582 * (
            1 - np.tanh((0.271020942398391 + -1.67288064390926 * P_MPa + 0.0673489649704582 * T)) ** 2
        )
        H7_PMPa = 0.0083174017874869 * (
            1 - np.tanh((-1.10142968268782 + 0.0083174017874869 * P_MPa + 0.0108662672630239 * T)) ** 2
        )
        H7_T = 0.0108662672630239 * (
            1 - np.tanh((-1.10142968268782 + 0.0083174017874869 * P_MPa + 0.0108662672630239 * T)) ** 2
        )
        H8_PMPa = 1.17456681024276 * (
            1 - np.tanh((1.90064900956529 + 1.17456681024276 * P_MPa + -0.0902336799991085 * T)) ** 2
        )
        H8_T = -0.0902336799991085 * (
            1 - np.tanh((1.90064900956529 + 1.17456681024276 * P_MPa + -0.0902336799991085 * T)) ** 2
        )
        H9_PMPa = -0.943525497821755 * (
            1 - np.tanh((-0.673064219823259 + -0.943525497821755 * P_MPa + 0.0734160926798799 * T)) ** 2
        )
        H9_T = 0.0734160926798799 * (
            1 - np.tanh((-0.673064219823259 + -0.943525497821755 * P_MPa + 0.0734160926798799 * T)) ** 2
        )
        H10_PMPa = -1.99739736255923 * (
            1 - np.tanh((-1.6061497898911 + -1.99739736255923 * P_MPa + 0.108199114343877 * T)) ** 2
        )
        H10_T = 0.108199114343877 * (
            1 - np.tanh((-1.6061497898911 + -1.99739736255923 * P_MPa + 0.108199114343877 * T)) ** 2
        )
        H11_PMPa = 0.388202467775416 * (
            1 - np.tanh((1.73534543350612 + 0.388202467775416 * P_MPa + -0.0616088045146524 * T)) ** 2
        )
        H11_T = -0.0616088045146524 * (
            1 - np.tanh((1.73534543350612 + 0.388202467775416 * P_MPa + -0.0616088045146524 * T)) ** 2
        )
        H12_PMPa = 1.3326759135354 * (
            1 - np.tanh((2.70166390032434 + 1.3326759135354 * P_MPa + -0.118211564739528 * T)) ** 2
        )
        H12_T = -0.118211564739528 * (
            1 - np.tanh((2.70166390032434 + 1.3326759135354 * P_MPa + -0.118211564739528 * T)) ** 2
        )
        H13_PMPa = 9.42488755971789 * (
            1 - np.tanh((-9.32984593347698 + 9.42488755971789 * P_MPa + 0.0784916020511085 * T)) ** 2
        )
        H13_T = 0.0784916020511085 * (
            1 - np.tanh((-9.32984593347698 + 9.42488755971789 * P_MPa + 0.0784916020511085 * T)) ** 2
        )
        H14_PMPa = 1.13189377766378 * (
            1 - np.tanh((1.2207418073194 + 1.13189377766378 * P_MPa + -0.0841043542652104 * T)) ** 2
        )
        H14_T = -0.0841043542652104 * (
            1 - np.tanh((1.2207418073194 + 1.13189377766378 * P_MPa + -0.0841043542652104 * T)) ** 2
        )
        H15_PMPa = 1.84291010959674 * (
            1 - np.tanh((2.58665026763982 + 1.84291010959674 * P_MPa + -0.13970364770731 * T)) ** 2
        )
        H15_T = -0.13970364770731 * (
            1 - np.tanh((2.58665026763982 + 1.84291010959674 * P_MPa + -0.13970364770731 * T)) ** 2
        )
...
def gh2_cp(P, T, deriv=False):
    ...
    if deriv:
        ...
        H1_PMPa = 1.13159708689763 * (
            1 - np.tanh((0.614735260887242 + 1.13159708689763 * P_MPa + -0.0551837204578247 * T)) ** 2
        )
        H1_T = -0.0551837204578247 * (
            1 - np.tanh((0.614735260887242 + 1.13159708689763 * P_MPa + -0.0551837204578247 * T)) ** 2
        )
        H2_PMPa = -0.0140493655427582 * (
            1 - np.tanh((-0.400739830511314 + -0.0140493655427582 * P_MPa + -0.000581742517018706 * T)) ** 2
        )
        H2_T = -0.000581742517018706 * (
            1 - np.tanh((-0.400739830511314 + -0.0140493655427582 * P_MPa + -0.000581742517018706 * T)) ** 2
        )
        H3_PMPa = -0.584213145968296 * (
            1 - np.tanh((-1.25439265281633 + -0.584213145968296 * P_MPa + 0.100382842687856 * T)) ** 2
        )
        H3_T = 0.100382842687856 * (
            1 - np.tanh((-1.25439265281633 + -0.584213145968296 * P_MPa + 0.100382842687856 * T)) ** 2
        )
        H4_PMPa = 1.1389617257106 * (
            1 - np.tanh((0.613718515601412 + 1.1389617257106 * P_MPa + -0.0766902288518594 * T)) ** 2
        )
        H4_T = -0.0766902288518594 * (
            1 - np.tanh((0.613718515601412 + 1.1389617257106 * P_MPa + -0.0766902288518594 * T)) ** 2
        )
        H5_PMPa = 1.12786935775064 * (
            1 - np.tanh((0.783863063691068 + 1.12786935775064 * P_MPa + -0.0580710892849312 * T)) ** 2
        )
        H5_T = -0.0580710892849312 * (
            1 - np.tanh((0.783863063691068 + 1.12786935775064 * P_MPa + -0.0580710892849312 * T)) ** 2
        )
        H6_PMPa = 6.11836677910538 * (
            1 - np.tanh((3.0862845706326 + 6.11836677910538 * P_MPa + -0.28171194186548 * T)) ** 2
        )
        H6_T = -0.28171194186548 * (
            1 - np.tanh((3.0862845706326 + 6.11836677910538 * P_MPa + -0.28171194186548 * T)) ** 2
        )
        H7_PMPa = -1.74531315193444 * (
            1 - np.tanh((-4.01267608558022 + -1.74531315193444 * P_MPa + 0.13245397217098 * T)) ** 2
        )
        H7_T = 0.13245397217098 * (
            1 - np.tanh((-4.01267608558022 + -1.74531315193444 * P_MPa + 0.13245397217098 * T)) ** 2
        )
        H8_PMPa = -2.69493649415449 * (
            1 - np.tanh((-4.73597371609159 + -2.69493649415449 * P_MPa + 0.241924487447467 * T)) ** 2
        )
        H8_T = 0.241924487447467 * (
            1 - np.tanh((-4.73597371609159 + -2.69493649415449 * P_MPa + 0.241924487447467 * T)) ** 2
        )
        H9_PMPa = -2.55750416599446 * (
            1 - np.tanh((-3.13544345816356 + -2.55750416599446 * P_MPa + 0.149566251986045 * T)) ** 2
        )
        H9_T = 0.149566251986045 * (
            1 - np.tanh((-3.13544345816356 + -2.55750416599446 * P_MPa + 0.149566251986045 * T)) ** 2
        )
        H10_PMPa = -4.95468689159338 * (
            1 - np.tanh((-3.57473472514032 + -4.95468689159338 * P_MPa + 0.281591654527403 * T)) ** 2
        )
        H10_T = 0.281591654527403 * (
            1 - np.tanh((-3.57473472514032 + -4.95468689159338 * P_MPa + 0.281591654527403 * T)) ** 2
        )
        H11_PMPa = 3.38001091079686 * (
            1 - np.tanh((4.41722437760112 + 3.38001091079686 * P_MPa + -0.242556425504075 * T)) ** 2
        )
        H11_T = -0.242556425504075 * (
            1 - np.tanh((4.41722437760112 + 3.38001091079686 * P_MPa + -0.242556425504075 * T)) ** 2
        )
        H12_PMPa = 0.0072937040171696 * (
            1 - np.tanh((-0.398001325855848 + 0.0072937040171696 * P_MPa + 0.000277104672759379 * T)) ** 2
        )
        H12_T = 0.000277104672759379 * (
            1 - np.tanh((-0.398001325855848 + 0.0072937040171696 * P_MPa + 0.000277104672759379 * T)) ** 2
        )
        H13_PMPa = 0.576128323850331 * (
            1 - np.tanh((-0.31379942783082 + 0.576128323850331 * P_MPa + 0.0347928080405698 * T)) ** 2
        )
        H13_T = 0.0347928080405698 * (
            1 - np.tanh((-0.31379942783082 + 0.576128323850331 * P_MPa + 0.0347928080405698 * T)) ** 2
        )
        H14_PMPa = 1.16723768888899 * (
            1 - np.tanh((7.34692537702712 + 1.16723768888899 * P_MPa + -0.317917699002537 * T)) ** 2
        )
        H14_T = -0.317917699002537 * (
            1 - np.tanh((7.34692537702712 + 1.16723768888899 * P_MPa + -0.317917699002537 * T)) ** 2
        )
        H15_PMPa = 0.784988883943678 * (
            1 - np.tanh((-0.816554474050802 + 0.784988883943678 * P_MPa + 0.0377876586456731 * T)) ** 2
        )
        H15_T = 0.0377876586456731 * (
            1 - np.tanh((-0.816554474050802 + 0.784988883943678 * P_MPa + 0.0377876586456731 * T)) ** 2
        )
...
def gh2_u(P, T, deriv=False):
    ...
    if deriv:
        ...
        H1_PMPa = 0.948951297504785 * (
            1 - np.tanh((0.233539747755315 + 0.948951297504785 * P_MPa + -0.0499745330828293 * T)) ** 2
        )
        H1_T = -0.0499745330828293 * (
            1 - np.tanh((0.233539747755315 + 0.948951297504785 * P_MPa + -0.0499745330828293 * T)) ** 2
        )
        H2_PMPa = 2.13093505892378 * (
            1 - np.tanh((2.05471470504367 + 2.13093505892378 * P_MPa + -0.119572061260962 * T)) ** 2
        )
        H2_T = -0.119572061260962 * (
            1 - np.tanh((2.05471470504367 + 2.13093505892378 * P_MPa + -0.119572061260962 * T)) ** 2
        )
        H3_PMPa = 0.587703772663913 * (
            1 - np.tanh((0.398892133197 + 0.587703772663913 * P_MPa + -0.00336363426965824 * T)) ** 2
        )
        H3_T = -0.00336363426965824 * (
            1 - np.tanh((0.398892133197 + 0.587703772663913 * P_MPa + -0.00336363426965824 * T)) ** 2
        )
        H4_PMPa = 0.992347038797521 * (
            1 - np.tanh((0.143237719165029 + 0.992347038797521 * P_MPa + -0.0143964824374372 * T)) ** 2
        )
        H4_T = -0.0143964824374372 * (
            1 - np.tanh((0.143237719165029 + 0.992347038797521 * P_MPa + -0.0143964824374372 * T)) ** 2
        )
        H5_PMPa = -2.74657042229242 * (
            1 - np.tanh((-3.96616131435837 + -2.74657042229242 * P_MPa + 0.186453818992668 * T)) ** 2
        )
        H5_T = 0.186453818992668 * (
            1 - np.tanh((-3.96616131435837 + -2.74657042229242 * P_MPa + 0.186453818992668 * T)) ** 2
        )
        H6_PMPa = -0.645489048931109 * (
            1 - np.tanh((1.44405264544866 + -0.645489048931109 * P_MPa + -0.0104476083443945 * T)) ** 2
        )
        H6_T = -0.0104476083443945 * (
            1 - np.tanh((1.44405264544866 + -0.645489048931109 * P_MPa + -0.0104476083443945 * T)) ** 2
        )
        H7_PMPa = 3.10126879273836 * (
            1 - np.tanh((2.02006732389655 + 3.10126879273836 * P_MPa + -0.132381179154892 * T)) ** 2
        )
        H7_T = -0.132381179154892 * (
            1 - np.tanh((2.02006732389655 + 3.10126879273836 * P_MPa + -0.132381179154892 * T)) ** 2
        )
        H8_PMPa = -0.295895155010423 * (
            1 - np.tanh((0.455915191247191 + -0.295895155010423 * P_MPa + -0.00270733720731719 * T)) ** 2
        )
        H8_T = -0.00270733720731719 * (
            1 - np.tanh((0.455915191247191 + -0.295895155010423 * P_MPa + -0.00270733720731719 * T)) ** 2
        )
        H9_PMPa = 0.135982980631872 * (
            1 - np.tanh((-1.42121486333193 + 0.135982980631872 * P_MPa + 0.0473542542463267 * T)) ** 2
        )
        H9_T = 0.0473542542463267 * (
            1 - np.tanh((-1.42121486333193 + 0.135982980631872 * P_MPa + 0.0473542542463267 * T)) ** 2
        )
        H10_PMPa = -0.206113999100671 * (
            1 - np.tanh((-0.662630708916202 + -0.206113999100671 * P_MPa + 0.00414455453741901 * T)) ** 2
        )
        H10_T = 0.00414455453741901 * (
            1 - np.tanh((-0.662630708916202 + -0.206113999100671 * P_MPa + 0.00414455453741901 * T)) ** 2
        )
        H11_PMPa = 1.67371525829477 * (
            1 - np.tanh((0.56740916395517 + 1.67371525829477 * P_MPa + -0.0218895513528701 * T)) ** 2
        )
        H11_T = -0.0218895513528701 * (
            1 - np.tanh((0.56740916395517 + 1.67371525829477 * P_MPa + -0.0218895513528701 * T)) ** 2
        )
        H12_PMPa = 0.1589500143037 * (
            1 - np.tanh((0.237517777100042 + 0.1589500143037 * P_MPa + -0.0106519272801902 * T)) ** 2
        )
        H12_T = -0.0106519272801902 * (
            1 - np.tanh((0.237517777100042 + 0.1589500143037 * P_MPa + -0.0106519272801902 * T)) ** 2
        )
        H13_PMPa = 0.754447306182754 * (
            1 - np.tanh((1.54182270996619 + 0.754447306182754 * P_MPa + -0.0541064232047049 * T)) ** 2
        )
        H13_T = -0.0541064232047049 * (
            1 - np.tanh((1.54182270996619 + 0.754447306182754 * P_MPa + -0.0541064232047049 * T)) ** 2
        )
        H14_PMPa = 1.72079624611917 * (
            1 - np.tanh((2.62921984539398 + 1.72079624611917 * P_MPa + -0.0971068814297633 * T)) ** 2
        )
        H14_T = -0.0971068814297633 * (
            1 - np.tanh((2.62921984539398 + 1.72079624611917 * P_MPa + -0.0971068814297633 * T)) ** 2
        )
        H15_PMPa = -0.484939056845666 * (
            1 - np.tanh((-1.04212680836281 + -0.484939056845666 * P_MPa + 0.0116518536560464 * T)) ** 2
        )
        H15_T = 0.0116518536560464 * (
            1 - np.tanh((-1.04212680836281 + -0.484939056845666 * P_MPa + 0.0116518536560464 * T)) ** 2
        )
...
def gh2_h(P, T, deriv=False):
    ...
    if deriv:
        ...
        H1_PMPa = -0.511704824190866 * (
            1 - np.tanh((1.73579390297524 + -0.511704824190866 * P_MPa + -0.00328063298246929 * T)) ** 2
        )
        H1_T = -0.00328063298246929 * (
            1 - np.tanh((1.73579390297524 + -0.511704824190866 * P_MPa + -0.00328063298246929 * T)) ** 2
        )
        H2_PMPa = -0.0787067450602815 * (
            1 - np.tanh((-0.17506458742044 + -0.0787067450602815 * P_MPa + 0.0043943295092826 * T)) ** 2
        )
        H2_T = 0.0043943295092826 * (
            1 - np.tanh((-0.17506458742044 + -0.0787067450602815 * P_MPa + 0.0043943295092826 * T)) ** 2
        )
        H3_PMPa = -2.75869130445277 * (
            1 - np.tanh((-1.49793344088779 + -2.75869130445277 * P_MPa + 0.110698824574587 * T)) ** 2
        )
        H3_T = 0.110698824574587 * (
            1 - np.tanh((-1.49793344088779 + -2.75869130445277 * P_MPa + 0.110698824574587 * T)) ** 2
        )
        H4_PMPa = 0.133730746273731 * (
            1 - np.tanh((-1.37669485784553 + 0.133730746273731 * P_MPa + 0.00649135107315252 * T)) ** 2
        )
        H4_T = 0.00649135107315252 * (
            1 - np.tanh((-1.37669485784553 + 0.133730746273731 * P_MPa + 0.00649135107315252 * T)) ** 2
        )
        H5_PMPa = -1.88672855275111 * (
            1 - np.tanh((-1.1405914153055 + -1.88672855275111 * P_MPa + 0.0981301522587858 * T)) ** 2
        )
        H5_T = 0.0981301522587858 * (
            1 - np.tanh((-1.1405914153055 + -1.88672855275111 * P_MPa + 0.0981301522587858 * T)) ** 2
        )
        H6_PMPa = 0.429585607469454 * (
            1 - np.tanh((0.446124093572671 + 0.429585607469454 * P_MPa + -0.0285787427235227 * T)) ** 2
        )
        H6_T = -0.0285787427235227 * (
            1 - np.tanh((0.446124093572671 + 0.429585607469454 * P_MPa + -0.0285787427235227 * T)) ** 2
        )
        H7_PMPa = 0.398927054757526 * (
            1 - np.tanh((-2.49683893292792 + 0.398927054757526 * P_MPa + 0.0128111795517157 * T)) ** 2
        )
        H7_T = 0.0128111795517157 * (
            1 - np.tanh((-2.49683893292792 + 0.398927054757526 * P_MPa + 0.0128111795517157 * T)) ** 2
        )
        H8_PMPa = -1.82172820931808 * (
            1 - np.tanh((-1.34037612046496 + -1.82172820931808 * P_MPa + 0.0975510622080853 * T)) ** 2
        )
        H8_T = 0.0975510622080853 * (
            1 - np.tanh((-1.34037612046496 + -1.82172820931808 * P_MPa + 0.0975510622080853 * T)) ** 2
        )
        H9_PMPa = -0.0840846337708169 * (
            1 - np.tanh((-0.00454329936396595 + -0.0840846337708169 * P_MPa + 0.00192700682148055 * T)) ** 2
        )
        H9_T = 0.00192700682148055 * (
            1 - np.tanh((-0.00454329936396595 + -0.0840846337708169 * P_MPa + 0.00192700682148055 * T)) ** 2
        )
        H10_PMPa = -0.817575328835318 * (
            1 - np.tanh((3.25989647210865 + -0.817575328835318 * P_MPa + -0.00790145526484188 * T)) ** 2
        )
        H10_T = -0.00790145526484188 * (
            1 - np.tanh((3.25989647210865 + -0.817575328835318 * P_MPa + -0.00790145526484188 * T)) ** 2
        )
        H11_PMPa = -5.5187467484401 * (
            1 - np.tanh((1.2138688950769 + -5.5187467484401 * P_MPa + 0.115937604475061 * T)) ** 2
        )
        H11_T = 0.115937604475061 * (
            1 - np.tanh((1.2138688950769 + -5.5187467484401 * P_MPa + 0.115937604475061 * T)) ** 2
        )
        H12_PMPa = -3.44784008416792 * (
            1 - np.tanh((-4.2841473959043 + -3.44784008416792 * P_MPa + 0.225314720754115 * T)) ** 2
        )
        H12_T = 0.225314720754115 * (
            1 - np.tanh((-4.2841473959043 + -3.44784008416792 * P_MPa + 0.225314720754115 * T)) ** 2
        )
        H13_PMPa = 0.59904718534549 * (
            1 - np.tanh((-0.749714454876193 + 0.59904718534549 * P_MPa + 0.015852118328226 * T)) ** 2
        )
        H13_T = 0.015852118328226 * (
            1 - np.tanh((-0.749714454876193 + 0.59904718534549 * P_MPa + 0.015852118328226 * T)) ** 2
        )
        H14_PMPa = -3.26552973737454 * (
            1 - np.tanh((-3.56659915773571 + -3.26552973737454 * P_MPa + 0.201884080139387 * T)) ** 2
        )
        H14_T = 0.201884080139387 * (
            1 - np.tanh((-3.56659915773571 + -3.26552973737454 * P_MPa + 0.201884080139387 * T)) ** 2
        )
        H15_PMPa = -4.14838409559996 * (
            1 - np.tanh((-0.368559368031245 + -4.14838409559996 * P_MPa + 0.1417710518596 * T)) ** 2
        )
        H15_T = 0.1417710518596 * (
            1 - np.tanh((-0.368559368031245 + -4.14838409559996 * P_MPa + 0.1417710518596 * T)) ** 2
        )
...
def lh2_cp(T, deriv=False):
    ...
    if deriv:
        return (
            ...
            / (0.0002684 - 7.6143e-6 * T - 2.5759e-7 * (T - 27.6691) ** 2) ** 2
        )
...
def sat_gh2_k(T, deriv=False):
    ...
    if deriv:
        return (
            ...
            / (110.21937 - 2.6596443 * T - 0.0153377 * (T - 27.6691) ** 2 - 0.0088632 * (T - 27.6691) ** 3) ** 2
        )
...
def sat_gh2_T(P, deriv=False):
    ...
    if deriv == 2:
        val = (
            -2 * 5.85e-12
            + 2 * 3 * 3.292e-18 * (P - 598825)
            - 3 * 4 * 1.246e-24 * (P - 598825) ** 2
            + 4 * 5 * 2.053e-29 * (P - 598825) ** 3
            - 5 * 6 * 3.463e-35 * (P - 598825) ** 4
        )
        val[P > 1235172] = 0.0
